...
app.post('/api/seed', async (req, res) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => db.run('DELETE FROM posts', err => err ? reject(err) : resolve()));
    for (const a of articles) {
      const slug = Math.random().toString(36).substring(2, 9);
      await new Promise((resolve, reject) => db.run(
        'INSERT INTO posts (theme, title, content, links, date, slug) VALUES (?, ?, ?, ?, ?, ?)',
        [a.theme, a.title, a.content, JSON.stringify(a.links), a.date, slug],
        err => err ? reject(err) : resolve()
      ));
    }
    db.close();
    res.json({ ok: true, inserted: articles.length });
  } catch (err) {
